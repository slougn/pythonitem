...
LastEditTime: 2022-05-10 23:15:57
LastEditors: 560130
Description: 直接导入H列数据,从H列开始,重新计算之后的列，并且完成动态规划
             本文档所有计算都使用numpy包
FilePath: /pythonitem/hgy/test/dynamic_programming_hgy.py
'''
import numpy as np
...
# 第J列  修正冷负荷 J = I * param
def J_column(I_c, param):
    J = []
    for i in range(len(I_c)):
        J.append(I_c[i] * param[i])
    return J
...
if __name__ == "__main__":
    ...
    # 目标函数
    ENERGY = np.array([215029 ,144621 ,167456 ,186485 ,226447 ,247379 ,277825 ,308272 ,249282 ,213126 ,203612 ,226447 ]) 
    BB = BB_caculate(param,H,CC,K)
