...
BUILDING = pd.read_excel(
    '/home/gree/wfDocument/gree/wfDocument/其他/练习项目/PythonItem/test/non_table.xlsx',
    sheet_name='R')
...
LOAD = pd.DataFrame(pd.read_excel('/home/gree/wfDocument/gree/wfDocument/其他/练习项目/PythonItem/test/non_table.xlsx',sheet_name='R'))
print(LOAD)
...
file_location = '/home/gree/wfDocument/gree/wfDocument/其他/练习项目/PythonItem/test/non_table.xlsx'
...
#保存原始冷负荷数据 H列，按照月份进行保存。 保存之后，在另一个文档中进行计算
np.save('1月',np.array(LOAD1_list0))
np.save('2月',np.array(LOAD2_list0))
np.save('3月',np.array(LOAD3_list0))
np.save('4月',np.array(LOAD4_list0))
np.save('5月',np.array(LOAD5_list0))
np.save('6月',np.array(LOAD6_list0))
np.save('7月',np.array(LOAD7_list0))
np.save('8月',np.array(LOAD8_list0))
np.save('9月',np.array(LOAD9_list0))
np.save('10月',np.array(LOAD10_list0))
np.save('11月',np.array(LOAD11_list0))
np.save('12月',np.array(LOAD12_list0))
...
def cool_machine2_load(P3,J3,O3):
    ...
    for i in range(len(P3)):
        if P3[i] == 0:
            ...
            R.append(J3[i]/(350*3.517*O3[i]+350*3.517*P3[i]))
    return R
...
def COP(X1,X2,X3): 
    ...
    d = 2.49432757221509
    return math.exp(a * X1 + b * X2 + c * X3 + d)
...
def machine1_power(V3,X3):
    ...
    for i in range(len(V3)):
        if V3[i] == 0:
            ...
            Z.append(X3[i]/V3[i])
    return Z
Z = machine1_power(V,X_cool_capacity)

# 2号功率
def machine2_power(W3,Y3):
    AA = machine1_power(W3,Y3)
    return AA
AA = machine2_power(W,Y_cool_capacity)

def machine_power(Z,AA):
    AT = []
    for i in range(len(Z)):
        AT.append(Z[i] + AA[i])
    return AT
AD = machine_power(Z,AA)
...
def machine1_flow(X3):
    ...
    return AG
AG = machine1_flow(X_cool_capacity)

# 2号冷冻水流量
def machine2_flow(Y3):
    AH = machine1_flow(Y3)
    return AH
AH = machine2_flow(Y_cool_capacity)
...
def lengque1_flow(X3):
    ...
    return AN
AN = lengque1_flow(X_cool_capacity)

# 2号冷却水流量
def lengque2_flow(Y3):
    AO = lengque1_flow(Y3)
    return AO
AO = lengque2_flow(Y_cool_capacity)
...
def terminal_power(AZ3,X3,Y3):
    ...
    return BA
BA = terminal_power(AZ,X_cool_capacity,Y_cool_capacity)
...
BB = air_conditioner_power(AD,AM,AT,AY,AZ,BA)
print(BB)
